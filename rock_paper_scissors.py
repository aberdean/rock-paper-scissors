...
Features 4 computer players with different strategies plus one human player.
One computer player is quite dumb and always plays "rock". Another one picks
a random move at each round, a third one picks the previous move made by the
opponent, and finally, the fourth one cycles through the moves from first to
last over and over, starting from a random one.
The rules of the game are that paper covers rock, rock crushes scissors,
and scissors cut paper.
The entire game runs on 7 rounds and who wins the greatest amount of rounds is
the final winner.

Attributes:
    MOVES (list): The list of moves available in the game
"""
